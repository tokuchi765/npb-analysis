...
func TestTeamReader_ReadTeamExchangeStats(t *testing.T) {
	...
	tests := []struct {
		name                         string
		args                         args
		wantTeamExchangeMatchResults []team.TeamMatchResults
	}{
		{
			"セリーグ交流戦CSV読み込み",
			args{
...
func TestTeamReader_ReadTeamPlayers(t *testing.T) {
	runtimeCurrent, _ := filepath.Abs("../../")
	tests := []struct {
		name        string
		initial     string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			actual := teamReader.ReadTeamPlayers(runtimeCurrent+"/test/resource", tt.initial, tt.teamName)
			assert.Exactly(t, tt.wantPlayers, actual)
		})
	}
}
